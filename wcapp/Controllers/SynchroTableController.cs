...
{
    public class SynchroTableController : Controller
    {
        public ActionResult Index(int? pageNo, string searchId, int? pageSize)
        {
            ...
            if (searchId != null)
            {
                int rowsCount = db.SynchroTables.Where(x => x.partNo.Contains(searchId)).Count();
                // int pCount = (int)Math.Ceiling(1.0 * rowsCount / pSize);//取天花板数
                // 调整当前页码不超过总页数
                int crtNo = Common.FixPageNo(pNo, pSize, rowsCount);
                crtNo = crtNo <= 0 ? 1 : crtNo;
                //总记录数
                List<SynchroTable> list = db.SynchroTables.Where(x => x.partNo.Contains(searchId)).OrderBy(a => a.importState).Skip((crtNo - 1) * pSize).Take(pSize).ToList();
                //总页数
                ...
                ViewBag.PageList = li;
                ViewBag.SynchroTable = list;
                return View();
            }
            else
            {
                int rowsCount = db.SynchroTables.Count();
                // int pCount = (int)Math.Ceiling(1.0 * rowsCount / pSize);//取天花板数
                // 调整当前页码不超过总页数
                int crtNo = Common.FixPageNo(pNo, pSize, rowsCount);
                crtNo = crtNo <= 0 ? 1 : crtNo;
                //总记录数
                List<SynchroTable> list = db.SynchroTables.OrderBy(a => a.importState).Skip((crtNo - 1) * pSize).Take(pSize).ToList();
                //总页数
                int pageCount = (rowsCount - 1) / pSize + 1;
                int nextPageNo = crtNo >= pageCount ? pageCount : crtNo + 1;//计算下一页页号
                int prevPageNo = crtNo == 1 ? 1 : crtNo - 1;//计算上一页页号
        ...
        [HttpPost]
        public ActionResult ImportSync(HttpPostedFileBase file)
        {
            try
            {
        ...
        public ActionResult ExportSynchroTable(string[] syncId)
        {
            try
            {
                if (GlobalData.IsDeviceNet)//内网导出
                {
                    ...
                    {
                        var db = new Context();
                        var Syncs = db.SynchroTables.Where(x => syncId.Contains(x.taskNo)).ToArray();

                        var folder = new DirectoryInfo(AppDomain.CurrentDomain.BaseDirectory + "Syncs");
                        if (!folder.Exists)
                        {
                            folder.Create();//如果文件夹不存在则创建一个新的
                        }
                        var fileName = Guid.NewGuid() + ".Sync";
                        var name = "Syncs/" + fileName;
                        new SyncFile(Syncs).SaveAs(AppDomain.CurrentDomain.BaseDirectory + name);
                        foreach (var sync in Syncs)
                        {
                            sync.importTime = DateTime.Now;
                            sync.importState = true;
                            db.SaveChanges();
                        }
