...
namespace WCAPP.Libs
{
    public class ERPFile
    {
        ...
        public ERPFile(Stream inputStream)
        {
            try
            {
                ...
                if (md5 != json.Md532())
                    throw new Exception("所选的ERP文件格式不正确！");
            }
            catch (Exception e)
            {
                throw new Exception("所选的ERP文件格式不正确！" + e.Message);
            }
        }
        public ERPFile(ERP[] erp)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            //serializer.RecursionLimit = 1;
            json = serializer.Serialize(erp);
        }
        ...
        public ERP[] ToERP()
        {
            return new JavaScriptSerializer().Deserialize<ERP[]>(json);
        }        
    }
}